...
class WordInputView(tk.Frame):
    ...
    def build_ui(self):
        ...
        self.entry_palabra = tk.Entry(form_frame, width=20, font=("Arial", 14), justify="center")
        self.entry_palabra.grid(row=0, column=1, padx=10, pady=10)

        btn_enviar = tk.Button(self, text="Enviar palabra", width=18, command=self.enviar_palabra)
    ...
    def enviar_palabra(self):
        ...
        if palabra == "":
            self.lbl_estado.config(text="Ingresa una palabra")
            return
